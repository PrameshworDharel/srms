...
if (strlen($_SESSION['alogin']) == "") {
    ...
    // Add this code before the HTML table
    $chartData = array();
    $sql = "SELECT tblclasses.ClassName, COUNT(tblstudents.StudentId) as TotalStudents
             FROM tblstudents
    ...
    if ($query->rowCount() > 0) {
        foreach ($results as $result) {
            $chartData[$result->ClassName] = $result->TotalStudents;
        }
    }
?>
                    ...
                    <section class="section">
                        <div class="container-fluid">
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12" style="margin-top:1%">
                                    <a class="dashboard-stat bg-success" href="manage-notices.php">
                                        <?php
                                            $sql3 = "SELECT COUNT(*) as totalNotices FROM tblnotice"; // Count the total number of notices
                                            $query3 = $dbh->prepare($sql3);
                                            $query3->execute();
                                            $totalNotices = $query3->fetch(PDO::FETCH_OBJ)->totalNotices; // Retrieve the total count

                                            ?>
                                        <span class="number counter">
                                            <?php echo htmlentities($totalNotices); ?>
                                        </span>
                                        <span class="name">Total Notice</span>
                                        <span class="bg-icon"><i class="fa fa-bell"></i></span>
                                    </a>
                                    <!-- /.dashboard-stat -->
                                </div>


                                <canvas id="classChart" style="width: 1200px; height: 200px;"></canvas>
