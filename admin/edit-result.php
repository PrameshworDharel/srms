...
if (strlen($_SESSION['alogin']) == "") {
    ...
    if (isset($_POST['submit'])) {
        ...
        foreach ($_POST['id'] as $count => $id) {
            ...
            $iid = $rowid[$count];
            // $eex=  $rowid[$count]
            // Perform validation: marks should not be greater than 100
            if ($mrks <= 100) {
                $sql = "UPDATE tblresult SET marks=:mrks WHERE id=:iid"; //,Exam=:exam
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <form class="form-horizontal" method="post">
                                            ...
                                            <?php
                                                ...
                                                $ret = "SELECT distinct tblstudents.StudentName,tblclasses.ClassName,tblclasses.Semister,tblresult.Exam from tblresult join tblstudents on tblresult.StudentId=tblresult.StudentId join tblsubjects on tblsubjects.id=tblresult.SubjectId join tblclasses on tblclasses.id=tblstudents.ClassId where tblstudents.StudentId=$stid  and tblresult.Exam ='$ex' limit 1";
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-10">
                                                    <?php echo htmlentities($row->Exam); ?>
                                                </div>
                                            </div>
                                            ...
                                            <?php
                                                $sql = "SELECT distinct tblstudents.StudentName,tblstudents.StudentId,tblclasses.ClassName,tblclasses.Semister,tblsubjects.SubjectName,tblresult.marks,tblresult.id as resultid from tblresult join tblstudents on tblstudents.StudentId=tblresult.StudentId join tblsubjects on tblsubjects.id=tblresult.SubjectId join tblclasses on tblclasses.id=tblstudents.ClassId where tblstudents.StudentId=$stid and tblresult.Exam ='$ex'";
                                                $query = parse_str($_SERVER[$sql], $stid, $ex);
                                                $query = $dbh->prepare($sql);
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-10">
                                                    ...
                                                    <input type="number" name="marks[]" class="form-control" id="marks"
                                                        value="<?php echo htmlentities($result->marks) ?>" min="0"
                                                        max="100" required="required" autocomplete="off">
                                                </div>
                                            </div>
