...
if (strlen($_SESSION['alogin']) == "") {
    ...
    if (isset($_POST['submit'])) {
        ...
        // Check if the combination already exists for the same class and subject
        $sql = "SELECT * FROM tblsubjectcombination WHERE ClassId = :class AND SubjectId = :subject";
        $query = $dbh->prepare($sql);
        $query->bindParam(':class', $class, PDO::PARAM_STR);
        $query->bindParam(':subject', $subject, PDO::PARAM_STR);
        $query->execute();

        // Check if the combination already exists for the same subject in another class semester
        if ($query->rowCount() > 0) {
            $error = "Combination already exists same class and subject.";
        } else {
            $sql = "SELECT * FROM tblsubjectcombination WHERE SubjectId = :subject AND ClassId != :class";
            $query = $dbh->prepare($sql);
            $query->bindParam(':subject', $subject, PDO::PARAM_STR);
            $query->bindParam(':class', $class, PDO::PARAM_STR);
            $query->execute();
            ...
            if ($query->rowCount() > 0) {
                $error = "Combination already exists  same subject .";
            } else {
                                                ...
                                                <div class="col-sm-10">
                                                    <select name="class" class="form-control" id="default"
                                                        ...
                                                        <option value="<?php echo htmlentities($result->id); ?>">
                                                            ...
                                                            Semister-<?php echo htmlentities($result->Semister); ?>
                                                        </option>
                                                        <?php }
                                                            } ?>
                                                    </select>
                                            ...
                                            <div class="form-group">
                                                <div class="col-sm-offset-2 col-sm-10">
                                                    <button type="submit" name="submit"
                                                        class="btn btn-success bg-primary"> Add</button>
                                                </div>
                                            </div>
                                        </form>
